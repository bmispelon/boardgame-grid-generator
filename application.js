...
function permutationToBinaryString (permutation) {
  const evenIndices = Array(Math.floor(permutation.length / 2)).fill().map((_, i) => 2 * i)
  const permutationAsTripletIndices = permutation.map(triplet => TRIPLETS.indexOf(triplet))
  return evenIndices.map(i => String.fromCharCode(16 * permutationAsTripletIndices[i] + permutationAsTripletIndices[i + 1])).join('')
}
